...
class DecisionHandoffService:
    ...
    @staticmethod
    def extract_tilts(bundle: Mapping[str, Any]) -> dict[str, Any]:
        """Pull PIT tilts from top-level or nested paper-engine snapshot."""
        snap = {}
        readiness = bundle.get("data_readiness") or {}
        if isinstance(readiness, Mapping):
            snap = readiness.get("paper_world_model_snapshot") or {}
        if not isinstance(snap, Mapping):
            snap = {}
        wmi = bundle.get("world_model_index") or {}
        if not isinstance(wmi, Mapping):
            wmi = {}
        macro = bundle.get("macro_tilt", snap.get("macro_tilt"))
        alt = bundle.get("alt_tilt", snap.get("alt_tilt"))
        return {
            "macro_tilt": macro,
            "alt_tilt": alt,
    ...
    def is_valve_open(self, bundle: Mapping[str, Any]) -> bool:
        wmi = bundle.get("world_model_index") or {}
        if isinstance(wmi, Mapping) and "should_ai_abstain" in wmi:
    ...
    def act(self, bundle: Mapping[str, Any]) -> dict[str, Any]:
        ...
        tilts = self.extract_tilts(bundle)
        ...
        if self.require_open_valve and not open_valve:
            return {
                ...
                "handoff": "blocked",
                **{k: tilts[k] for k in ("band_scope", "archive_complete", "wmi", "full_schema_wmi")},
            }

        macro = self._tilt_sign(tilts.get("macro_tilt"))
        alt = self._tilt_sign(tilts.get("alt_tilt"))
        score = macro + alt
        if score > 0:
            action = "bullish"
        elif score < 0:
        ...
        return {
            ...
            "macro_tilt": tilts.get("macro_tilt"),
            "alt_tilt": tilts.get("alt_tilt"),
            "macro_tilt_sign": macro,
            "alt_tilt_sign": alt,
            "evidence_ids": evidence
            or [
                f"tilt:macro:{tilts.get('macro_tilt')}",
                f"tilt:alt:{tilts.get('alt_tilt')}",
            ],
            "handoff": "acted",
            **{k: tilts[k] for k in ("band_scope", "archive_complete", "wmi", "full_schema_wmi")},
        }
