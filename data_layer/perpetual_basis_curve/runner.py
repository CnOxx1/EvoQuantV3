...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="永续基差曲线数据模块运行入口")
    parser.add_argument(
...
def main():
    ...
    setup_logger("perpetual_basis_curve")
...
def _run_blocking_scheduler(service, symbols):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 perpetual_basis_curve...")
        try:
    ...
    logger.info("perpetual_basis_curve 调度器已启动（BlockingScheduler, interval=60min）")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service, symbols):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 perpetual_basis_curve 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            pass
    ...
    signal.signal(signal.SIGTERM, shutdown)
    scheduler.start()
    logger.info("perpetual_basis_curve 异步调度器已启动")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
