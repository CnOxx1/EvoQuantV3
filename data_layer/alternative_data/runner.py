...
def main():
    ...
    if args.async_scheduler:
        _run_async_scheduler(service, source_names, entity_keys)
    else:
        _run_blocking_scheduler(service, source_names, entity_keys)


def _run_blocking_scheduler(service, source_names, entity_keys):
    ...
    logger.info("alternative_data 调度器已启动（BlockingScheduler），按 Ctrl+C 停止")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service, source_names, entity_keys):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 alternative_data 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            logger.debug("调度器已经停止，无需重复关闭")
    ...
    scheduler.start()
    logger.info("alternative_data 异步调度器已启动（AsyncIOScheduler），按 Ctrl+C 停止")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
