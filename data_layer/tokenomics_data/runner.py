...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="tokenomics 数据模块运行入口")
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--interval",
        type=str,
        default=TOKENOMICS_CONFIG["default_interval"],
        help="标准化频率，例如 1d",
    )
    parser.add_argument(
        "--lookback-hours",
        type=int,
        default=TOKENOMICS_CONFIG["default_lookback_hours"],
        help="每次请求回看小时数",
    )
    parser.add_argument(
        "--skip-initial-run",
...
def _format_sources(rows: list[dict]) -> str:
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"factor={row['primary_factor_id']} entity_type={row['entity_type']}: "
            f"{row['description']}"
        )
    return "\n".join(lines)
...
def _run_blocking_scheduler(service, entity_keys, interval, lookback_hours):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 tokenomics_data 模块...")
        try:
    ...
    logger.info("tokenomics_data 调度器已启动（BlockingScheduler），按 Ctrl+C 停止")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service, entity_keys, interval, lookback_hours):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 tokenomics_data 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            logger.debug("调度器已经停止，无需重复关闭")
    ...
    scheduler.start()
    logger.info("tokenomics_data 异步调度器已启动（AsyncIOScheduler），按 Ctrl+C 停止")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
