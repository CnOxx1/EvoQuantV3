...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="交易所公告采集模块运行入口")
    parser.add_argument(
        "--mode", choices=["bootstrap", "once", "scheduler"], default="once",
        help="bootstrap: 初始化回填；once: 单次采集；scheduler: 定时采集（每15分钟）",
    )
...
def main():
    ...
    setup_logger("exchange_announcement")
...
def _run_blocking_scheduler(service):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 exchange_announcement...")
        try:
    ...
    logger.info("exchange_announcement 调度器已启动（BlockingScheduler），每15分钟采集")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 exchange_announcement 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            pass
    ...
    signal.signal(signal.SIGTERM, shutdown)
    scheduler.start()
    logger.info("exchange_announcement 异步调度器已启动")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
