...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CeFi 借贷利率数据模块运行入口")
    parser.add_argument(
        "--mode", choices=["bootstrap", "once", "scheduler"], default="once",
        help="bootstrap: 回填数据；once: 单次采集；scheduler: 定时采集（每60分钟）",
    )
    parser.add_argument("--assets", type=str, default="", help="按资产过滤，逗号分隔")
...
def main():
    ...
    setup_logger("cefi_lending_rate")
...
def _run_blocking_scheduler(service, assets):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 cefi_lending_rate...")
        try:
    ...
    logger.info("cefi_lending_rate 调度器已启动（BlockingScheduler），每60分钟采集")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service, assets):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 cefi_lending_rate 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            pass
    ...
    signal.signal(signal.SIGTERM, shutdown)
    scheduler.start()
    logger.info("cefi_lending_rate 异步调度器已启动")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
