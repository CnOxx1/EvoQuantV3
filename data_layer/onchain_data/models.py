...
class OnchainSourceDefinition(BaseModel):
    """链上来源定义。"""
    ...
    factor_id: str = Field(..., description="主因子 ID")
    entity_type: str = Field(..., description="实体类型")
    default_interval: str = Field(default="1h", description="默认频率")
...
class OnchainFactorDefinition(BaseModel):
    """链上因子目录定义。"""
    ...
    staleness_ttl_seconds: int = Field(default=7200, description="新鲜度 TTL")
...
class OnchainTimeSeriesPoint(BaseModel):
    """标准化后的链上因子时序点。"""
    ...
    interval: str = Field(..., description="标准化频率，例如 15m / 1h / 1d")
