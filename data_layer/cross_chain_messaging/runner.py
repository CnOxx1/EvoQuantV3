...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="跨链消息协议数据模块运行入口")
    parser.add_argument(
...
def main():
    ...
    setup_logger("cross_chain_messaging")
...
def _run_blocking_scheduler(service):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 cross_chain_messaging...")
        try:
    ...
    logger.info("cross_chain_messaging 调度器已启动（BlockingScheduler），每10分钟采集")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 cross_chain_messaging 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            pass
    ...
    signal.signal(signal.SIGTERM, shutdown)
    scheduler.start()
    logger.info("cross_chain_messaging 异步调度器已启动")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
