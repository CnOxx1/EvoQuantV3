...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DEX 流动性数据模块运行入口")
    parser.add_argument(
        "--mode", choices=["bootstrap", "once", "scheduler"], default="once",
        help="bootstrap: 回填数据；once: 单次采集；scheduler: 定时采集（每20分钟）",
    )
    ...
    parser.add_argument(
        "--protocol", choices=["uniswap_v3", "curve", "all"], default="all",
        help="指定采集协议（默认全部）",
    )
    return parser
...
def main():
    ...
    setup_logger("dex_liquidity_data")
...
def _run_blocking_scheduler(service):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 dex_liquidity_data...")
        try:
    ...
    logger.info("dex_liquidity_data 调度器已启动（BlockingScheduler），每20分钟采集")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 dex_liquidity_data 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            pass
    ...
    signal.signal(signal.SIGTERM, shutdown)
    scheduler.start()
    logger.info("dex_liquidity_data 异步调度器已启动")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
