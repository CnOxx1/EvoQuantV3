...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="监管动态数据模块运行入口")
    parser.add_argument(
        "--mode", choices=["bootstrap", "once", "scheduler"], default="once",
        help="bootstrap: 初始化 ETF 追踪并回填；once: 单次采集；scheduler: 定时采集（每 2 小时）",
    )
...
def _run_blocking_scheduler(service):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 regulatory_data...")
        try:
    ...
    logger.info("regulatory_data 调度器已启动（BlockingScheduler），每 2 小时采集")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 regulatory_data 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            pass
    ...
    signal.signal(signal.SIGTERM, shutdown)
    scheduler.start()
    logger.info("regulatory_data 异步调度器已启动")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
