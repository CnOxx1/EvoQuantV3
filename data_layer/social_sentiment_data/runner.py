...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="社交情绪数据模块运行入口")
    parser.add_argument(
        "--mode",
        choices=["bootstrap", "once", "scheduler"],
        default="once",
        help="bootstrap: 回填近 7 天数据；once: 执行一次采集；scheduler: 启动定时采集",
    )
    parser.add_argument(
        "--symbols",
        type=str,
        default="",
        help="按标的过滤，逗号分隔，例如 BTC,ETH,SOL",
    )
    parser.add_argument(
        "--async-scheduler",
        action="store_true",
        help="scheduler 模式下使用 AsyncIOScheduler",
    )
    parser.add_argument(
        "--print-context",
        action="store_true",
        help="输出 AI 上下文 bundle",
    )
    return parser
...
def main():
    ...
    setup_logger("social_sentiment_data")
    service = SocialSentimentDataService()
...
def _run_blocking_scheduler(service, symbols):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 social_sentiment_data 模块...")
        try:
    ...
    logger.info("social_sentiment_data 调度器已启动（BlockingScheduler）")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service, symbols):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 social_sentiment_data 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            pass
    ...
    scheduler.start()
    logger.info("social_sentiment_data 异步调度器已启动（AsyncIOScheduler）")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
