...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="代币解锁实时数据模块运行入口")
    parser.add_argument(
...
def main():
    ...
    setup_logger("token_unlock_realtime")
...
def _run_blocking_scheduler(service):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 token_unlock_realtime...")
        try:
    ...
    logger.info("token_unlock_realtime 调度器已启动（BlockingScheduler），每60分钟采集")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 token_unlock_realtime 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            pass
    ...
    signal.signal(signal.SIGTERM, shutdown)
    scheduler.start()
    logger.info("token_unlock_realtime 异步调度器已启动")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
