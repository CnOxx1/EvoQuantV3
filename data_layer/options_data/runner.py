...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="期权数据模块运行入口")
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument("--sources", type=str, default="", help="按来源过滤，逗号分隔")
    parser.add_argument("--factors", type=str, default="", help="按 factor_id 过滤，逗号分隔")
    parser.add_argument("--entities", type=str, default="", help="按实体过滤，逗号分隔")
    parser.add_argument(
        "--interval",
        type=str,
        default=OPTIONS_CONFIG["default_interval"],
        help="标准化频率，例如 1h",
    )
    parser.add_argument(
        "--lookback-hours",
        type=int,
        default=OPTIONS_CONFIG["default_lookback_hours"],
        help="每次请求回看小时数",
    )
    parser.add_argument(
        "--skip-initial-run",
    ...
    parser.add_argument("--list-sources", action="store_true", help="列出 source 注册表")
    parser.add_argument("--list-factors", action="store_true", help="列出 factor 注册表")
    parser.add_argument("--list-entities", action="store_true", help="列出 entity 注册表")
    parser.add_argument("--print-context", action="store_true", help="输出 AI 上下文 bundle")
    parser.add_argument("--print-coverage", action="store_true", help="输出采集覆盖情况")
    return parser
...
def _format_sources(rows: list[dict]) -> str:
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"collector={row['collector_key']} entity_type={row['entity_type']} "
            f"semantic_scope={row['semantic_scope']}: {row['description']}"
        )
    return "\n".join(lines)
...
def main():
    ...
    if args.async_scheduler:
        _run_async_scheduler(service, entity_keys, args.interval, args.lookback_hours)
    else:
        _run_blocking_scheduler(service, entity_keys, args.interval, args.lookback_hours)


def _run_blocking_scheduler(service, entity_keys, interval, lookback_hours):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 options_data 模块...")
        try:
    ...
    logger.info("options_data 调度器已启动（BlockingScheduler），按 Ctrl+C 停止")
    try:
        scheduler.start()
    finally:
        service.close()
...
def _run_async_scheduler(service, entity_keys, interval, lookback_hours):
    ...
    def shutdown(signum, frame):
        logger.info("收到关闭信号，正在停止 options_data 异步调度器...")
        try:
            scheduler.shutdown(wait=False)
        except Exception:
            logger.debug("调度器已经停止，无需重复关闭")
    ...
    scheduler.start()
    logger.info("options_data 异步调度器已启动（AsyncIOScheduler），按 Ctrl+C 停止")
    try:
        loop.run_forever()
    finally:
        scheduler.shutdown(wait=False)
