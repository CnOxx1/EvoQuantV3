...
class OptionsSourceDefinition(BaseModel):
...
class OptionsFactorDefinition(BaseModel):
    ...
    staleness_ttl_seconds: int = Field(default=14400, description="新鲜度 TTL")
...
class OptionsTimeSeriesPoint(BaseModel):
    factor_id: str = Field(..., description="系统内部稳定因子 ID")
    category: str = Field(..., description="因子大类")
    factor_type: str = Field(..., description="指标语义类型")
    entity_type: str = Field(..., description="实体类型")
