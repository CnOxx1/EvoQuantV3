...
from data_layer.options_data.models import OptionsTimeSeriesPoint


class OptionsCollectorBase:
    """期权时序点公共落库逻辑。"""
    ...
    @staticmethod
    def _quality_rank(point: OptionsTimeSeriesPoint) -> tuple:
        quality_rank = {
            "ok": 3,
            "partial": 2,
            "fallback": 1,
    ...
    @staticmethod
    def _history_identity(point: OptionsTimeSeriesPoint) -> tuple:
        return (
            point.factor_id,
            point.entity_type,
            point.entity_key,
    ...
    @staticmethod
    def _latest_identity(point: OptionsTimeSeriesPoint) -> tuple:
        return (
            point.factor_id,
            point.entity_type,
            point.entity_key,
    ...
    def _deduplicate_history_points(
        ...
        deduped: dict[tuple, OptionsTimeSeriesPoint] = {}
        for point in points:
    ...
    def save_to_db(self, points: list[OptionsTimeSeriesPoint]):
        points = self._deduplicate_history_points(points)
        if not points:
            return

        history_sql = """
            INSERT INTO options_timeseries (
        ...
        """
        latest_sql = """
            INSERT INTO latest_options_timeseries (
                factor_id, category, factor_type, entity_type, entity_key,
                interval, observation_time, value, unit, quality_flag,
                dimensions_key, dimensions_json, config_version, source_name,
                source_symbol, raw_payload_json, collected_at
            ...
            ON CONFLICT(
                ...
                collected_at=excluded.collected_at,
                updated_at=CURRENT_TIMESTAMP
            WHERE excluded.observation_time >= latest_options_timeseries.observation_time
        """
